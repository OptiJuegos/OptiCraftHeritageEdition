                                        ...
                                        const Ps2TerrainGpuState& gpu)
{
    ...
    // the mapping's near plane would otherwise run past 0x7FFF into the sign
    // bit of the signed PSMZ_16S buffer.
                                         ...
                                         const Ps2Vu1TerrainCommonState& state)
{
    Ps2VifQword* chain = reserveChain(queue,
        PS2_VU1_TERRAIN_CONTEXT_QWORDS + 2 + kPassStateChainQwords);
    if (chain == nullptr)
        return false;
    ...
    passState[0] = dmaTag(DMA_TAG_CNT, kPassStateChainQwords - 1);
    commandQword(passState[1],
                 vifCode(kVifNop, 0, 0),
                 vifCode(kVifNop, 0, 0),
                 vifCode(kVifNop, 0, 0),
                 vifCode(kVifDirect, 0, (unsigned int)(kPassStateRegisters + 1)));
    const unsigned long long passStateTag =
        GIF_TAG(kPassStateRegisters, 1, 0, 0, 0, 1);
