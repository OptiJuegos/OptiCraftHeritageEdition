...
public:
    ...
    std::istream* toStreamIn() const override
    {
        if (Ps2SaveFileSystem::isDisabledPath(u8path) || Ps2SaveFileSystem::isMemoryCardPath(u8path))
            return nullptr;
        std::unique_ptr<std::istream> input = PlatformStorage::openStdioInputStream(u8path);
    ...
    std::ostream* toStreamOut() const override
    {
        ...
        if (Ps2SaveFileSystem::isDisabledPath(u8path) || Ps2SaveFileSystem::isMemoryCardPath(u8path))
            return PlatformStorage::openDiscardOutputStream().release();

        std::unique_ptr<std::ostream> output = PlatformStorage::openStdioOutputStream(u8path);
