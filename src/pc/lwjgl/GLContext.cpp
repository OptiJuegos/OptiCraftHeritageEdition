...
public:
	GLContext()
	{
		...
		// Request a 24-bit depth buffer. SDL defaults to 16 bits, which gives
		// poor z precision and makes Advanced OpenGL occlusion queries
		// (GL_SAMPLES_PASSED) misbehave -> chunks falsely culled / black holes.
		...
		// Some drivers expose a lower multisample pixel format than the value
		// requested in options.txt. Retry lower C6 sample counts rather than
		// making an unsupported AA setting prevent the game from starting.
		...
		for (int sample : sampleCandidates)
		{
			...
			window = SDL_CreateWindow("Release++", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
