...
byte_t  Minecraft::field_28006_b[64 * 1024] = {};
#else
...
void Minecraft::startGame()
{
    ...
    MC_LOG_DEBUG("client.config", "renderDistance=%d preload=%d blocks cache=%d unload=%d "
           "gen=%d/tick mesh=%d/%dms warmup=%dms\n",
    ...
    PLATFORM_BOOT_LOG(PLATFORM_BOOT_PREFIX " loadScreen begin\n");
    if (ClientPlatformPolicy::showInitialLoadScreen())
        loadScreen();
    PLATFORM_BOOT_LOG(PLATFORM_BOOT_PREFIX " loadScreen done\n");
    ...
    // OptiFine: registrar las animaciones de textura segun los toggles (sin variantes HD).
    // Si no se registran, la textura estatica de terrain.png permanece (= animacion OFF).
...
void Minecraft::changeWorld(World *world, const std::string &s, EntityPlayerSP *entityplayer)
{
...
#if PLATFORM_LOAD_TERRAIN_WARMUP_MS > 0
    // RenderGlobal has just queued every section in the new renderer grid.
    // Build the nearby meshes while the loading screen is still on-screen so
    // the first play session does not spend its first frames compiling terrain
    // and allocating GX display-list buffers.  updateRenderers() retains its
    // normal MeshBudget, and this wall-clock cap leaves pathological/corrupt
    // saves with the usual incremental fallback instead of an endless load.
        ...
        renderGlobal != nullptr)
    {
        ...
        // LOCAL MODIFICATION (BetaPlusPlus Wii port): how much work was still
        // queued when the warmup loop started, so "capped" can be logged with
        // a sense of scale (X of Y sections built) instead of just a bool.
        // Diagnostic captures previously showed
        // "warmup capped" with no way to tell whether it fell one section
        // short or a few hundred -- which matters, because a first-ever load
        // of a session still has to build the native GX lists from scratch,
        // while a reload can benefit from already-warm game/resource caches.
        ...
        do
        {
            meshesReady = renderGlobal->updateRenderers(thePlayer, false);
            ...
            if (warmupChunkProvider != nullptr)
                warmupChunkProvider->serviceAsyncChunkStreaming();
            if (!meshesReady)
                PlatformCompat::delay(1);
#endif

            const long_t elapsed = System::currentTimeMillis() - warmupStart;
            const int_t progress = (int_t)((elapsed * 100L) /
                PLATFORM_LOAD_TERRAIN_WARMUP_MS);
            loadingScreen->setLoadingProgress(progress > 100 ? 100 : progress);
        }
        while (!meshesReady && System::currentTimeMillis() < warmupEnd);
...
void Minecraft::preloadWorld(const std::string &s)
{
    ...
    // Console gameplay generation is intentionally throttled/asynchronous, but
    // this loading-screen loop is the bounded synchronous preload. Reuse the
    // spawn-generation bypass so every requested column is resident before the
    // initial renderer warmup starts instead of returning temporary blank chunks.
