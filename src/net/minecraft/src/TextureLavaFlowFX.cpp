...
TextureLavaFlowFX::TextureLavaFlowFX() :
	TextureFX(0),
	maxFrames(0)
{
	iconIndex = Block::lavaMoving->blockIndexInTexture + 1;
	tileSize = 2;
	for (int_t i = 0; i < 256; i++)
	{
		textureHeight[i] = 0.0f;
		textureWidth[i] = 0.0f;
		frameCounter[i] = 0.0f;
		frameTime[i] = 0.0f;
	}
...
bool TextureLavaFlowFX::isAnimationEnabled() const
{
...
void TextureLavaFlowFX::onTick()
{
		...
		for (int_t j = 0; j < 16; j++)
		{
			...
			for (int_t k1 = i - 1; k1 <= i + 1; k1++)
			{
				for (int_t i2 = j - 1; i2 <= j + 1; i2++)
				{
					int_t k2 = k1 + l & 0xf;
					int_t i3 = i2 + i1 & 0xf;
					f += textureHeight[k2 + i3 * 16];
				}
			}
			textureWidth[i + j * 16] = f / 10.0f + ((frameCounter[(i + 0 & 0xf) + (j + 0 & 0xf) * 16]
				+ frameCounter[(i + 1 & 0xf) + (j + 0 & 0xf) * 16]
				+ frameCounter[(i + 1 & 0xf) + (j + 1 & 0xf) * 16]
				+ frameCounter[(i + 0 & 0xf) + (j + 1 & 0xf) * 16]) / 4.0f) * 0.8f;
			frameCounter[i + j * 16] += frameTime[i + j * 16] * 0.01f;
			if (frameCounter[i + j * 16] < 0.0f)
			{
				frameCounter[i + j * 16] = 0.0f;
			}
			frameTime[i + j * 16] -= 0.06f;
			if (Math::random() < 0.0050000000000000001)
			{
				frameTime[i + j * 16] = 1.5f;
			}
		}
	}
	...
	{
		float f1 = textureHeight[k - (maxFrames / 3) * 16 & 0xff] * 2.0f;
		if (f1 > 1.0f) f1 = 1.0f;
		if (f1 < 0.0f) f1 = 0.0f;
		float f2 = f1;
