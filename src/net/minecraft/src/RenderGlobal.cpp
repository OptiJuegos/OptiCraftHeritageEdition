...
#if PLATFORM_PC
	occlusionEnabled = renderSupportsFeature(RenderFeature::OcclusionQuery);
	// Desktop 1.2.5 retains three GL lists per WorldRenderer (two terrain passes
	// plus the occlusion box). Native console terrain has its own mesh handles and
	// never allocates this synthetic GL namespace.
...
void RenderGlobal::loadRenderers()
{
	...
	// Low-console profile: use a small fixed horizontal/vertical renderer grid.
	// Do not derive this from the PC render-distance table.
...
void RenderGlobal::markRenderersForNewPosition(int_t i, int_t j, int_t k)
{
	...
	for (int_t j1 = 0; j1 < renderChunksWide; j1++)
	{
		...
		for (int_t i2 = 0; i2 < renderChunksDeep; i2++)
		{
			...
			{
#if PLATFORM_CENTER_VERTICAL_RENDERERS
				// Minecraft 1.2.5 keeps desktop renderers fixed at Y 0..255. Low consoles
				// waste time drawing underground sections, so keep a small moving
				// vertical window centered around the player.
				// j was shifted by -8 above for the horizontal toroidal-grid math.
				// Undo that offset here or a three-section window sits one band too
				// low whenever the player is in the lower half of a section.
				int_t centerSection = JavaArithmetic::intShr(j + 8, 4);
				int_t startSection = centerSection - (renderChunksTall / 2);
				if (startSection < 0) startSection = 0;
				if (startSection > WorldHeight::SECTION_COUNT - renderChunksTall) startSection = WorldHeight::SECTION_COUNT - renderChunksTall;
				// Record so block-edit marking can map section->slot identically.
				verticalStartSection = startSection;
				int_t i3 = (startSection + l2) * 16;
#else
				int_t i3 = l2 * 16;
#endif
...
int_t RenderGlobal::renderSortedRenderers(int_t i, int_t j, int_t k, double d)
{
	...
	for (int_t i1 = i; i1 < j; i1++)
	{
		if (k == 0)
		{
			...
			else if (useOcclusion && !sortedWorldRenderers[i1]->isVisible)
			{
		...
		if (sortedWorldRenderers[i1]->skipRenderPass(k) || !sortedWorldRenderers[i1]->isInFrustum || (useOcclusion && !sortedWorldRenderers[i1]->isVisible))
			continue;
		...
		renderBatchRenderers.push_back(sortedWorldRenderers[i1]);
		l++;
#else
		int_t j1 = sortedWorldRenderers[i1]->getGLCallListForPass(k);
		if (j1 >= 0)
		{
			renderBatchRenderers.push_back(sortedWorldRenderers[i1]);
			l++;
	...
	if (k >= 0 && k <= 1 && ++s_ps2TerrainLogTick[k] >= 120)
	{
		...
		if (k == 0)
		{
			...
			MC_LOG_DEBUG("ps2", "terrain pass0 rendered=%d/%d total=%d loaded=%d skip=%d"
			       " skipPending=%d visible=%d clip=%d occ=%d pending=%d\n",
...
void RenderGlobal::renderSky(float f)
{
	if (worldObj->worldProvider->worldType == 1)
	{
		...
		for (int_t face = 0; face < 6; ++face)
		{
			...
			tessellator->startDrawingQuads();
			tessellator->setColorOpaque_I(1579032);
...
bool RenderGlobal::updateRenderers(EntityLiving *entityliving, bool flag)
{
...
#if PLATFORM_PS2 || PLATFORM_WII
	requestedUpdateLimit = std::max(requestedUpdateLimit, (int_t)PLATFORM_MAX_RENDERER_UPDATES_PER_FRAME);
#endif
	...
	{
		// Finishing an active staging mesh is more valuable than starting another
		// one, especially on PS2 where staging slots come from a fixed pool.
...
#if PLATFORM_PS2
	// Evaluate source readiness once per renderer, not from the sort comparator.
	// A comparator-based check multiplies the 3x3 chunkExists sweep by O(n log n)
	// comparisons, which is the wrong trade on the EE. Partitioning is linear and
	// ensures every renderer that can really mesh sits before generation-gated work.
	auto sourceReadyEnd = std::partition(rendererUpdateCandidates.begin(),
		rendererUpdateCandidates.end(), [](WorldRenderer *renderer)
		{
			return renderer->terrainSourcesReady();
		});
	const std::size_t sourceReadyCount = static_cast<std::size_t>(
		std::distance(rendererUpdateCandidates.begin(), sourceReadyEnd));
	const std::size_t sortedReadyCount = std::min<std::size_t>(
		sourceReadyCount, static_cast<std::size_t>(attemptLimit));
	if (sortedReadyCount > 0)
	{
		std::partial_sort(rendererUpdateCandidates.begin(),
			rendererUpdateCandidates.begin() + sortedReadyCount,
			sourceReadyEnd, rendererPriority);
	}

	if (sourceReadyCount < static_cast<std::size_t>(attemptLimit) && sourceReadyEnd != rendererUpdateCandidates.end())
	{
		const std::size_t gatedSlots = static_cast<std::size_t>(attemptLimit) - sourceReadyCount;
		const std::size_t gatedCount = static_cast<std::size_t>(
			std::distance(sourceReadyEnd, rendererUpdateCandidates.end()));
		const std::size_t sortedGatedCount = std::min(gatedSlots, gatedCount);
		std::partial_sort(sourceReadyEnd, sourceReadyEnd + sortedGatedCount,
	...
	if (++s_ps2UpdateLogTick >= 120)
	{
		...
		MC_LOG_DEBUG("ps2", "renderer work: attempts=%ld steps=%ld complete=%ld pending=%d\n",
...
#if PLATFORM_PS2 || PLATFORM_WII
				// Promote an already-dirty renderer too. It may have been waiting
				// behind terrain streaming when this direct block edit arrived. Active
				// console builds retain their staging work: PS2 independently compares
				// its source-chunk availability each step, avoiding global restart
				// storms as chunks stream around the player.
