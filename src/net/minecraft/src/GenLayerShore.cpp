...
GenLayerShore::GenLayerShore(long_t seed, Ptr parentLayer) : GenLayer(seed)
{
    parent = std::move(parentLayer);
}

GenLayer::IntArray &GenLayerShore::getInts(int_t x, int_t z, int_t width, int_t height)
{
        ...
        for (int_t localX = 0; localX < width; ++localX)
        {
            ...
            if (center == BiomeGenBase::mushroomIsland->biomeID)
            {
                result[localX + localZ * width] =
                    north != BiomeGenBase::ocean->biomeID && east != BiomeGenBase::ocean->biomeID &&
                    west != BiomeGenBase::ocean->biomeID && south != BiomeGenBase::ocean->biomeID
                        ? center
                        : BiomeGenBase::mushroomIslandShore->biomeID;
            }
            else if (center != BiomeGenBase::ocean->biomeID &&
                     center != BiomeGenBase::river->biomeID &&
            ...
            {
                result[localX + localZ * width] =
                    north == center && east == center && west == center && south == center
                        ? center
