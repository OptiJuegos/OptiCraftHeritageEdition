...
namespace
{
                                     ...
                                     double movementX, double movementZ)
    {
        ...
        ChunkProvider* asyncProvider = dynamic_cast<ChunkProvider*>(chunkProvider);
#endif
        int forwardX = 0;
        int forwardZ = 0;
        ...
        while (queued < PLATFORM_PREFETCH_CHUNKS_PER_STEP)
        {
            ...
            {
                const ChunkProvider::ChunkRequestStatus status = asyncProvider->requestChunkDetailed(bestX, bestZ);
...
EntityRenderer::EntityRenderer(Minecraft* minecraft)
{
    ...
    lightmapTexture = minecraft->renderEngine->allocateAndSetupTexture(&lightmapImage);
#endif
