...
bool readCompoundFile(const std::string& path, std::unique_ptr<NBTTagCompound>& out)
{
    ...
    {
#ifdef PS2_PLATFORM
        // World metadata is small and is needed before any chunk streaming starts.
        // Parse it through the contiguous gzip path on PS2 so tutorial validation
        // does not depend on the low-memory streambuf used for larger runtime NBT.
        const std::vector<char> compressed(bytes.begin(), bytes.end());
        out.reset(CompressedStreamTools::decompress(compressed));
#else
    ...
    {
        MC_LOG_ERROR("save", "Failed reading NBT '%s': %s\n", path.c_str(), e.what());
        out.reset();
        return false;
    }
    catch (...)
    {
        MC_LOG_ERROR("save", "Failed reading NBT '%s': unknown exception\n", path.c_str());
        out.reset();
        return false;
    }
}
