...
void World::endPopulationFastPath()
{
    ...
    for (int_t index = 0; index < 4; ++index)
    {
        ...
        {
            chunk->skylightRegenPending = false;
...
{
    if (y < 0 || y >= WorldHeight::HEIGHT ||
        x < -30000000 || z < -30000000 || x >= 30000000 || z >= 30000000)
        return 0;
	                                  ...
	                                  int_t expectedId, int_t newId)
{
    ...
    if (populationFastPathActive && lightEquivalent && simpleBlocks && callbackFreeReplacement)
    {
        const int_t cx = JavaArithmetic::intShr(x, 4);
        const int_t cz = JavaArithmetic::intShr(z, 4);
        const int_t dx = cx - populationFastBaseX;
        ...
        if (dx >= 0 && dx < 2 && dz >= 0 && dz < 2)
        {
            Chunk *chunk = populationFastChunks[dz * 2 + dx];
            if (chunk != nullptr)
                return chunk->replaceBlockIDForPopulation(
...
void World::scheduleBlockUpdate(int x, int y, int z, int blockId, int delay)
{
    ...
    if (blockId > 0)
    {
        entry->setScheduledTime(JavaArithmetic::longAdd(static_cast<long_t>(delay), worldInfo->getWorldTime()));
    }
...
void World::updateEntities()
{
    ...
    for (size_t j = 0; j < unloadedEntityList.size(); j++)
    {
        ...
        if (entity->addedToChunk && chunkExists(chunkX, chunkZ))
        {
            getChunkFromChunkCoords(chunkX, chunkZ)->removeEntity(entity);
        }
    }
...
void World::updateEntityWithOptionalForce(Entity* entity, bool flag)
{
    ...
    if (flag && !checkChunksExist(JavaArithmetic::intSub(i, RANGE), 0, JavaArithmetic::intSub(j, RANGE),
    ...
    for (int x = minX; x < maxX; x++)
    {
        for (int y = minY; y < maxY; y++)
        {
            for (int z = minZ; z < maxZ; z++)
            {
                ...
                double d1 = (float)(y + 1) - BlockFluid::getPercentAir(getBlockMetadata(x, y, z));
                if ((double)maxY >= d1)
                {
                    flag = true;
...
void World::markTileEntityChunkModified(int_t x, int_t y, int_t z, TileEntity *tileentity)
{
    ...
    for (IWorldAccess *access : worldAccesses)
        if (access != nullptr)
            access->doNothingWithTileEntity(x, y, z, tileentity);
}
        ...
        while (lightingToUpdate.size() > 0)
        {
            ...
            if (PLATFORM_LIGHTING_BUDGET_US > 0)
            {
                ...
                if (nowUs > budgetStartUs &&
                    nowUs - budgetStartUs >= (uint64_t)PLATFORM_LIGHTING_BUDGET_US)
                {
                    lightingUpdatesCounter--;
                    return true;
                }
...
void World::updateLightByType(EnumSkyBlock *type, int_t x, int_t y, int_t z)
{
    ...
    // The low-memory console backend deliberately keeps Beta++'s deferred and
...
void World::updateBlocksAndPlayCaveSounds()
{
    ...
    // How many of them this tick. Vanilla (and PC) visits all of them; console
    // takes a rotating slice so the phase cost is spread instead of spiking.
    ...
    {
        std::uint64_t chunkKey = s_tickOrder[(s_tickCursor + visited) % totalChunks];
        int_t pairChunkX = unpackChunkCoordX(chunkKey);
        int_t pairChunkZ = unpackChunkCoordZ(chunkKey);
...
std::vector<Entity*> &World::getEntitiesWithinAABBExcludingEntity(Entity* entity, AxisAlignedBB* aabb)
{
    ...
    for (int cx = minChunkX; cx <= maxChunkX; cx++)
    {
        for (int cz = minChunkZ; cz <= maxChunkZ; cz++)
        {
            if (chunkExists(cx, cz))
            {
                getChunkFromChunkCoords(cx, cz)->getEntitiesWithinAABBForEntity(entity, aabb, entitiesWithinAABBExcludingEntity);
            }
        }
    }
...
std::vector<Entity*> World::getEntitiesWithinAABB(const std::type_info& classType, AxisAlignedBB* aabb)
{
    ...
    for (int cx = minChunkX; cx <= maxChunkX; cx++)
    {
        for (int cz = minChunkZ; cz <= maxChunkZ; cz++)
        {
            if (chunkExists(cx, cz))
            {
                getChunkFromChunkCoords(cx, cz)->getEntitiesOfTypeWithinAAAB(classType, aabb, result);
            }
        }
    }
