...
ComponentNetherBridgeCorridor2::ComponentNetherBridgeCorridor2(
...
void ComponentNetherBridgeCorridor2::buildComponent(
	StructureComponent *parent, std::vector<StructureComponent *> &components, Random &random)
{
	getNextComponentZ(static_cast<ComponentNetherBridgeStartPiece *>(parent), components, random, 0, 1, true);
}

ComponentNetherBridgeCorridor2 *ComponentNetherBridgeCorridor2::createValidComponent(
	...
	int_t x, int_t y, int_t z, int_t direction, int_t depth)
{
	...
	return isAboveGround(&bounds) && StructureComponent::findIntersecting(components, bounds) == nullptr
		? new ComponentNetherBridgeCorridor2(depth, random, bounds, direction)
		: nullptr;
}

bool ComponentNetherBridgeCorridor2::addComponentParts(
	World *world, Random &, const StructureBoundingBox &b)
{
	...
	fillWithBlocks(world, b, 1, 2, 4, 4, 5, 4, brick, brick, false);
	fillWithBlocks(world, b, 1, 3, 4, 1, 4, 4, fence, brick, false);
	fillWithBlocks(world, b, 3, 3, 4, 3, 4, 4, fence, brick, false);
