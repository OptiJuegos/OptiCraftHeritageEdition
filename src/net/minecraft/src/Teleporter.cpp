#include "Teleporter.h"

#include "Block.h"
#include "BlockPortal.h"
#include "Entity.h"
#include "Material.h"
#include "MathHelper.h"
#include "World.h"
#include "WorldProvider.h"

Teleporter::Teleporter()
	: field_4232_a()
{
}

void Teleporter::placeInPortal(World *world, Entity *entity)
{
	if (world->worldProvider->worldType != 1)
	{
		if (!placeInExistingPortal(world, entity))
		{
			createPortal(world, entity);
			placeInExistingPortal(world, entity);
		}
		return;
	}

	const int_t baseX = MathHelper::floor_double(entity->posX);
	const int_t baseY = MathHelper::floor_double(entity->posY) - 1;
	const int_t baseZ = MathHelper::floor_double(entity->posZ);
	const int_t axisX = 1;
	const int_t axisZ = 0;

	for (int_t side = -2; side <= 2; ++side)
	{
		for (int_t forward = -2; forward <= 2; ++forward)
		{
			for (int_t dy = -1; dy < 3; ++dy)
			{
				const int_t x = baseX + forward * axisX + side * axisZ;
				const int_t y = baseY + dy;
				const int_t z = baseZ + forward * axisZ - side * axisX;
				world->setBlockWithNotify(x, y, z, dy < 0 ? Block::obsidian->blockID : 0);
			}
		}
	}

	entity->setLocationAndAngles((double)baseX, (double)baseY, (double)baseZ, entity->rotationYaw, 0.0f);
	entity->motionX = entity->motionY = entity->motionZ = 0.0;
}

bool Teleporter::placeInExistingPortal(World *world, Entity *entity)
{
	int_t c = 128;
	double d = -1.0;
	int_t i = 0;
	int_t j = 0;
	int_t k = 0;
	int_t l = MathHelper::floor_double(entity->posX);
	int_t i1 = MathHelper::floor_double(entity->posZ);
	for (int_t j1 = l - c; j1 <= l + c; j1++)
	{
		double d1 = ((double)j1 + 0.5) - entity->posX;
		for (int_t j2 = i1 - c; j2 <= i1 + c; j2++)
		{
			double d3 = ((double)j2 + 0.5) - entity->posZ;
			for (int_t k2 = 127; k2 >= 0; k2--)
			{
				if (world->getBlockId(j1, k2, j2) != Block::portal->blockID)
					continue;
				for (; world->getBlockId(j1, k2 - 1, j2) == Block::portal->blockID; k2--) {}
				double d5 = ((double)k2 + 0.5) - entity->posY;
				double d7 = d1 * d1 + d5 * d5 + d3 * d3;
				if (d < 0.0 || d7 < d)
				{
					d = d7;
					i = j1;
					j = k2;
					k = j2;
				}
			}
		}
	}

	if (d < 0.0)
		return false;

	int_t k1 = i;
	int_t l1 = j;
	int_t i2 = k;
	double d2 = (double)k1 + 0.5;
	double d4 = (double)l1 + 0.5;
	double d6 = (double)i2 + 0.5;
	if (world->getBlockId(k1 - 1, l1, i2) == Block::portal->blockID)
		d2 -= 0.5;
	if (world->getBlockId(k1 + 1, l1, i2) == Block::portal->blockID)
		d2 += 0.5;
	if (world->getBlockId(k1, l1, i2 - 1) == Block::portal->blockID)
		d6 -= 0.5;
	if (world->getBlockId(k1, l1, i2 + 1) == Block::portal->blockID)
		d6 += 0.5;
	entity->setLocationAndAngles(d2, d4, d6, entity->rotationYaw, 0.0f);
	entity->motionX = entity->motionY = entity->motionZ = 0.0;
	return true;
}

bool Teleporter::createPortal(World *world, Entity *entity)
{
	int_t byte0 = 16;
	double d = -1.0;
	int_t i = MathHelper::floor_double(entity->posX);
	int_t j = MathHelper::floor_double(entity->posY);
	int_t k = MathHelper::floor_double(entity->posZ);
	int_t l = i;
	int_t i1 = j;
	int_t j1 = k;
	int_t k1 = 0;
	int_t l1 = field_4232_a.nextInt(4);
	for (int_t i2 = i - byte0; i2 <= i + byte0; i2++)
	{
		double d1 = ((double)i2 + 0.5) - entity->posX;
		for (int_t j3 = k - byte0; j3 <= k + byte0; j3++)
		{
			double d3 = ((double)j3 + 0.5) - entity->posZ;
			for (int_t k4 = 127; k4 >= 0; k4--)
			{
				if (!world->isAirBlock(i2, k4, j3))
					continue;
				for (; k4 > 0 && world->isAirBlock(i2, k4 - 1, j3); k4--) {}
				bool broke0 = false;
				for (int_t k5 = l1; k5 < l1 + 4 && !broke0; k5++)
				{
					int_t l6 = k5 % 2;
					int_t i8 = 1 - l6;
					if (k5 % 4 >= 2)
					{
						l6 = -l6;
						i8 = -i8;
					}
					for (int_t j9 = 0; j9 < 3 && !broke0; j9++)
					{
						for (int_t k10 = 0; k10 < 4 && !broke0; k10++)
						{
							for (int_t l11 = -1; l11 < 4; l11++)
							{
								int_t j12 = i2 + (k10 - 1) * l6 + j9 * i8;
								int_t l12 = k4 + l11;
								int_t j13 = (j3 + (k10 - 1) * i8) - j9 * l6;
								if ((l11 < 0 && !world->getBlockMaterial(j12, l12, j13)->isSolid()) || (l11 >= 0 && !world->isAirBlock(j12, l12, j13)))
								{
									broke0 = true;
									break;
								}
							}
						}
					}
					if (broke0)
						continue;
					double d5 = ((double)k4 + 0.5) - entity->posY;
					double d7 = d1 * d1 + d5 * d5 + d3 * d3;
					if (d < 0.0 || d7 < d)
					{
						d = d7;
						l = i2;
						i1 = k4;
						j1 = j3;
						k1 = k5 % 4;
					}
				}
			}
		}
	}

	if (d < 0.0)
	{
		for (int_t j2 = i - byte0; j2 <= i + byte0; j2++)
		{
			double d2 = ((double)j2 + 0.5) - entity->posX;
			for (int_t k3 = k - byte0; k3 <= k + byte0; k3++)
			{
				double d4 = ((double)k3 + 0.5) - entity->posZ;
				for (int_t l4 = 127; l4 >= 0; l4--)
				{
					if (!world->isAirBlock(j2, l4, k3))
						continue;
					for (; world->isAirBlock(j2, l4 - 1, k3); l4--) {}
					bool broke1 = false;
					for (int_t l5 = l1; l5 < l1 + 2 && !broke1; l5++)
					{
						int_t i7 = l5 % 2;
						int_t j8 = 1 - i7;
						for (int_t k9 = 0; k9 < 4 && !broke1; k9++)
						{
							for (int_t l10 = -1; l10 < 4; l10++)
							{
								int_t i12 = j2 + (k9 - 1) * i7;
								int_t k12 = l4 + l10;
								int_t i13 = k3 + (k9 - 1) * j8;
								if ((l10 < 0 && !world->getBlockMaterial(i12, k12, i13)->isSolid()) || (l10 >= 0 && !world->isAirBlock(i12, k12, i13)))
								{
									broke1 = true;
									break;
								}
							}
						}
						if (broke1)
							continue;
						double d6 = ((double)l4 + 0.5) - entity->posY;
						double d8 = d2 * d2 + d6 * d6 + d4 * d4;
						if (d < 0.0 || d8 < d)
						{
							d = d8;
							l = j2;
							i1 = l4;
							j1 = k3;
							k1 = l5 % 2;
						}
					}
				}
			}
		}
	}

	int_t k2 = k1;
	int_t l2 = l;
	int_t i3 = i1;
	int_t l3 = j1;
	int_t i4 = k2 % 2;
	int_t j4 = 1 - i4;
	if (k2 % 4 >= 2)
	{
		i4 = -i4;
		j4 = -j4;
	}
	if (d < 0.0)
	{
		if (i1 < 70) i1 = 70;
		if (i1 > 118) i1 = 118;
		i3 = i1;
		for (int_t i5 = -1; i5 <= 1; i5++)
			for (int_t i6 = 1; i6 < 3; i6++)
				for (int_t j7 = -1; j7 < 3; j7++)
				{
					int_t k8 = l2 + (i6 - 1) * i4 + i5 * j4;
					int_t l9 = i3 + j7;
					int_t i11 = (l3 + (i6 - 1) * j4) - i5 * i4;
					bool flag = j7 < 0;
					world->setBlockWithNotify(k8, l9, i11, flag ? Block::obsidian->blockID : 0);
				}
	}
	for (int_t j5 = 0; j5 < 4; j5++)
	{
		world->editingBlocks = true;
		for (int_t j6 = 0; j6 < 4; j6++)
			for (int_t k7 = -1; k7 < 4; k7++)
			{
				int_t l8 = l2 + (j6 - 1) * i4;
				int_t i10 = i3 + k7;
				int_t j11 = l3 + (j6 - 1) * j4;
				bool flag1 = j6 == 0 || j6 == 3 || k7 == -1 || k7 == 3;
				world->setBlockWithNotify(l8, i10, j11, flag1 ? Block::obsidian->blockID : Block::portal->blockID);
			}

		world->editingBlocks = false;
		for (int_t k6 = 0; k6 < 4; k6++)
			for (int_t l7 = -1; l7 < 4; l7++)
			{
				int_t i9 = l2 + (k6 - 1) * i4;
				int_t j10 = i3 + l7;
				int_t k11 = l3 + (k6 - 1) * j4;
				world->notifyBlocksOfNeighborChange(i9, j10, k11, world->getBlockId(i9, j10, k11));
			}
	}
	return true;
}
