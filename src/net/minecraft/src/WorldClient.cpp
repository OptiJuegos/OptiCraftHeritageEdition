...
{
#if PLATFORM_MP_DEFERRED_CHUNKS
    while (deferredChunkBytes > PLATFORM_MP_COMPRESSED_CHUNK_CACHE_BYTES && !deferredChunks.empty())
    {
        ...
        ulong_t oldestStamp = std::numeric_limits<ulong_t>::max();
        for (auto it = deferredChunks.begin(); it != deferredChunks.end(); ++it)
        {
            const DeferredChunk &entry = it->second;
            const long_t distance = getChunkDistance(entry.chunkX, entry.chunkZ, centerX, centerZ);
            if (distance > victimDistance ||
                (distance == victimDistance && entry.stamp < oldestStamp))
            {
                victim = it;
...
void WorldClient::promoteDeferredChunks()
{
    ...
    auto inflatePacket = [](const std::vector<byte_t> &compressed, int_t primaryMask,
                            bool includeInitialize, std::vector<byte_t> &out) -> bool
    {
        int_t sectionCount = 0;
        for (int_t section = 0; section < 16; ++section)
            sectionCount += (primaryMask >> section) & 1;
        const std::size_t outputSize = 12288u * (std::size_t)sectionCount
            + (includeInitialize ? 256u : 0u);
        if (compressed.empty() || outputSize == 0)
            return false;
        out.assign(outputSize, 0);
        uLongf actual = (uLongf)out.size();
    ...
    for (int_t promoted = 0; promoted < PLATFORM_MP_CHUNK_PROMOTIONS_PER_TICK; ++promoted)
    {
        ...
        DeferredChunk &entry = best->second;
        std::vector<byte_t> data;
        if (!inflatePacket(entry.compressed, entry.primaryMask, true, data))
        {
        ...
        for (const DeferredMapUpdate &update : entry.sectionUpdates)
        {
            if (!inflatePacket(update.compressed, update.primaryMask, false, data) ||
