...
ComponentNetherBridgeCorridor::ComponentNetherBridgeCorridor(
...
void ComponentNetherBridgeCorridor::buildComponent(
	StructureComponent *parent, std::vector<StructureComponent *> &components, Random &random)
{
	getNextComponentX(static_cast<ComponentNetherBridgeStartPiece *>(parent), components, random, 0, 1, true);
}

ComponentNetherBridgeCorridor *ComponentNetherBridgeCorridor::createValidComponent(
	...
	int_t x, int_t y, int_t z, int_t direction, int_t depth)
{
	...
	return isAboveGround(&bounds) && StructureComponent::findIntersecting(components, bounds) == nullptr
		? new ComponentNetherBridgeCorridor(depth, random, bounds, direction)
		: nullptr;
}

bool ComponentNetherBridgeCorridor::addComponentParts(
	World *world, Random &, const StructureBoundingBox &b)
{
	...
	fillWithBlocks(world, b, 0, 2, 4, 3, 5, 4, brick, brick, false);
