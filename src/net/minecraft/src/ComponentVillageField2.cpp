...
ComponentVillageField2::ComponentVillageField2(
...
void ComponentVillageField2::buildComponent(
	StructureComponent *, std::vector<StructureComponent *> &, Random &)
{
}

ComponentVillageField2 *ComponentVillageField2::findValidPlacement(
	...
	int_t x, int_t y, int_t z, int_t direction, int_t type)
{
	auto bounds = std::make_unique<StructureBoundingBox>(
		StructureBoundingBox::getComponentToAddBoundingBox(x, y, z, 0, 0, 0, 7, 4, 9, direction));
	if (!canVillageGoDeeper(bounds.get()) || StructureComponent::findIntersecting(components, *bounds) != nullptr)
		return nullptr;
	return new ComponentVillageField2(type, random, std::move(bounds), direction);
}

bool ComponentVillageField2::addComponentParts(
	World *world, Random &random, const StructureBoundingBox &chunkBounds)
{
	...
	fillWithBlocks(world, chunkBounds, 0, 1, 0, 6, 4, 8, 0, 0, false);
	...
	fillWithBlocks(world, chunkBounds, 1, 0, 0, 5, 0, 0, Block::wood->blockID, Block::wood->blockID, false);
	fillWithBlocks(world, chunkBounds, 1, 0, 8, 5, 0, 8, Block::wood->blockID, Block::wood->blockID, false);
