# ps2.cmake — PlayStation 2 (Emotion Engine) build branch for BetaPlusPlus.
#
...
# Optional direct VU1 terrain microprogram.  The renderer keeps a complete
# native VU0/gsKit fallback, so a toolchain without dvp-as remains buildable.
...
# --- Target -------------------------------------------------------------------
add_executable(BetaPlusPlus ${PS2_SOURCES})
set_target_properties(BetaPlusPlus PROPERTIES SUFFIX ".elf")
...
if(NOT MC_LOG_SYNC_WRITES STREQUAL "")
    target_compile_definitions(BetaPlusPlus PRIVATE MC_LOG_SYNC_WRITES=${MC_LOG_SYNC_WRITES})
endif()
if(NOT MC_LOG_COMMIT_EVERY STREQUAL "")
    target_compile_definitions(BetaPlusPlus PRIVATE MC_LOG_COMMIT_EVERY=${MC_LOG_COMMIT_EVERY})
endif()
...
target_link_libraries(BetaPlusPlus
...
set(_PS2_LINKED_SIZE_REPORT "${CMAKE_BINARY_DIR}/BetaPlusPlus.linked-size.txt")
set(_PS2_PACKAGED_SIZE_REPORT "${CMAKE_BINARY_DIR}/BetaPlusPlus.size.txt")
...
if(CMAKE_BUILD_TYPE STREQUAL "Release")
    add_custom_command(TARGET BetaPlusPlus POST_BUILD
        ...
        COMMAND ${CMAKE_COMMAND} -E copy_if_different
            "$<TARGET_FILE:BetaPlusPlus>" "${PS2_APP_DIR}/BetaPlusPlus.elf"
        COMMENT "Validating, stripping and packaging ${PS2_APP_DIR}/BetaPlusPlus.elf"
        VERBATIM
    )
else()
    add_custom_command(TARGET BetaPlusPlus POST_BUILD
        ...
        COMMAND ${CMAKE_COMMAND} -E copy_if_different
            "$<TARGET_FILE:BetaPlusPlus>" "${PS2_APP_DIR}/BetaPlusPlus.elf"
        COMMENT "Validating and packaging ${PS2_APP_DIR}/BetaPlusPlus.elf"
        VERBATIM
    )
endif()
